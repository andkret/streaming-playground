...
export default function PipelineVisualization({ messageRate, dbType, setDbType }) {
  ...
  // Calculate overflows and unified lag
  ...
  const lag = Math.max(kafkaOverflow, sparkOverflow, dbOverflow);

  // Target and actual sent
  const targetRate = messageRate;
  const actualSent = Math.max(0, targetRate - kafkaOverflow);

  // Processed messages for Spark (minus DB lag, capped by capacity)
  const processed = Math.min(Math.max(0, actualSent - lag), sparkCapacity);
  ...
  let batchDuration;
  if (dbOverflow > 0 && writeSpeed === 0) {
    batchDuration = 30;
  } else if (dbOverflow > 0) {
    batchDuration = Math.min(30, 1 + dbOverflow * 0.0005);
  } else if (sparkOverflow > 0) {
    batchDuration = Math.min(60, 1 + sparkOverflow * 0.0005);
  } else {
    batchDuration = 1;
  }
  ...
  const nodes = [
    {
      id: 'producer',
      type: 'pipelineNode',
      data: { label: (<><strong>Producer</strong><div>Target Rate: {targetRate.toLocaleString()} msg/s</div><div>Actual Sent: {actualSent.toLocaleString()} msg/s</div></>) },
      position: { x: 50, y: 100 },
    },
    {
      id: 'kafka',
      type: 'pipelineNode',
      data: { label: (<><strong>Kafka Broker</strong><div>Lag: {lag.toLocaleString()} msg</div></>) },
      position: { x: 300, y: 100 },
    },
    {
      id: 'spark',
      type: 'pipelineNode',
      data: { label: (<><strong>Spark Processor</strong><div>Processed: {processed.toLocaleString()} msg/s</div></>) },
      position: { x: 550, y: 100 },
    },
    {
      id: 'db',
      type: 'pipelineNode',
      data: { label: (<><strong>Database Sink</strong><div>Missing Messages: {lag.toLocaleString()} msg</div></>) },
